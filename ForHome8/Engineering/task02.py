...
from ForHome8.DataScience.task03_data import samples, species
...
def clusterize(X, name, y=None, n_features = 2, scaling=True, pca = True, tsne = True, truncatedsvd = False, args = None, export_to_file = True, ws = None, diagrams_dir='./diagrams', real_labels = None, clusterization_distance = 1):
    ...
    if 'hdbscan' in name:
        ...
        if args == None:
            param_grid = {
                'kmeans__max_iter': [500, 1000, 5000],
                'kmeans__n_clusters':np.arange(15)
                }
        else:
            param_grid = args['params']
        reg = KMeans()

        steps.append(('kmeans', reg))
...
def main():
    X = pd.read_csv('./ForHome8/DataScience/eurovision_voting.csv', index_col=0)

    X.columns = X.columns.astype(str)

    filename = './ForHome8/Engineering/model_report_task02_engineering.xlsx'
    ...
    for name in model_options:
        for preprocessing in data_preprocessing:
            for n_component in n_components:
                ...
                clusterize(X=X, name=name, n_features=n_component, scaling=scaling, pca=pca, tsne=tsne, truncatedsvd=truncatedsvd, ws=ws, clusterization_distance=12, diagrams_dir=diagrams_folder_path)
