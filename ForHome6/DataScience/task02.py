...
def main():
    ...
    X_options = [
        ...
        df.drop(columns=['mean radius', 'mean perimeter', 'mean area', 'mean smoothness', 'worst radius', 'worst radius', 'worst perimeter', 'worst area', 'worst smoothness']), #removing all highly correlated data
        df.drop(columns=['radius error', 'perimeter error', 'area error']), #removing all highly correlated data
        df.drop(columns=['mean compactness', 'mean concave points', 'worst compactness', 'worst concavity', 'worst concave points']), #removing all highly correlated data
        df.drop(columns=['mean radius', 'mean perimeter', 'mean area', 'mean smoothness', 'worst radius', 'worst radius', 'worst perimeter', 'worst area', 'worst smoothness', 'radius error', 'perimeter error', 'area error']), #removing all highly correlated data
        df.drop(columns=['mean radius', 'mean perimeter', 'mean area', 'mean smoothness', 'worst radius', 'worst radius', 'worst perimeter', 'worst area', 'worst smoothness', 'radius error', 'perimeter error', 'area error', 'mean compactness', 'mean concave points', 'worst compactness', 'worst concavity', 'worst concave points']), #removing all highly correlated data
        df.drop(columns=[ 'mean perimeter', 'mean area', 'mean smoothness', 'worst radius', 'worst radius', 'worst perimeter', 'worst area', 'worst smoothness', 'perimeter error', 'area error', 'mean concave points', 'worst compactness', 'worst concavity', 'worst concave points']), #removing all highly correlated data
    ...
    # ensemble_models_params = [
    #     {
    #         'ensemble__logistic__C': np.arange(.1,1, .1),
    #         'ensemble__logistic__penalty': ['l2'],
    #         'ensemble__svm__kernel':['rbf', 'polynomial', 'linear'],
    #         'ensemble__svm__C': np.arange(.1,1, .1),
    #         'ensemble__svm__gamma': ['scale'],
    #         'ensemble__cart__criterion':['gini','entropy'],
    #         'ensemble__cart__max_depth': np.arange(1, 100, 1),
    #         'ensemble__cart__min_samples_split': np.arange(2, 50, 1)
    #     }
    # ]
