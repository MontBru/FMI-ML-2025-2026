...
def train_and_test_model(X, y, ws, name, args = None, export_to_file = True):
    ...
    if 'logistic' in name:
        ...
        if args == None:
            param_grid = {
                'cart__criterion':['gini_index','entropy'],
        ...
        for model in models:
            ...
            trained_models.append((model, cv.best_estimator_))
    ...
    if export_to_file:
        ...
        plt.tight_layout()
        plt.savefig(f'{diagrams_folder}/{name}_confusion_matrix_{X_test.columns[0]}.png')
        plt.cla()

        img = openpyxl.drawing.image.Image(f'{diagrams_folder}/{name}_confusion_matrix_{X_test.columns[0]}.png')
...
def main():
    ...
    filename = './ForHome6/Engineering/model_report_engineering_task03.xlsx'
    global diagrams_folder 
    diagrams_folder = './ForHome6/Engineering/diagrams'
    ...
    plt.savefig(f'{diagrams_folder}/roc_curve.png')
    plt.cla()

    img = openpyxl.drawing.image.Image(f'{diagrams_folder}/roc_curve.png')
    img.anchor = 'A7'
    ws.add_image(img)
