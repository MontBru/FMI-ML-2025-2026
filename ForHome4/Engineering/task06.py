from ml_lib.linear_model import LinearRegression, Ridge, Lasso
import pandas as pd
...
def train_and_test_model(X, y, ws, name):
    ...
    cv = GridSearchCV(reg, param_grid=param_grid,cv=kf, return_train_score=True)
    ...
    plt.tight_layout()
    plt.savefig(f'{dir}/{name}_regression_diagram_{X_test.columns[0]}.png')
    plt.cla()

    img = openpyxl.drawing.image.Image(f'{dir}/{name}_regression_diagram_{X_test.columns[0]}.png')
...
def main():
    ...
    filename = './ForHome4/Engineering/model_report_task06_engineering.xlsx'

    wb = openpyxl.Workbook()
    wb.create_sheet('ModelReport')
    ws = wb['ModelReport']

    ws.append(['Model', 'Number of variables','Hyperparams', 'R2 score', "R2 score increase from base model (in %)"])

    y = df['glucose']

    X_options = [
        df[['diabetes','insulin']],
        df[['diabetes', 'diastolic','triceps','insulin','bmi','dpf','age']],
    ]
