...
def train_and_test_model(X, y, ws, name):
    ...
    plt.tight_layout()
    plt.savefig(f'./ForHome5/DataScience/diagrams/{name}_confusion_matrix.png')
    plt.cla()

    img = openpyxl.drawing.image.Image(f'ForHome5/DataScience/diagrams/{name}_confusion_matrix.png')
...
def main():
    df = pd.read_csv('./ForHome5/music_clean.csv')
    filename = './ForHome5/model_report_task03_engineering.xlsx'
    ...
    y = df['popularity'] > avg_popularity
    ...
    X_options = [
        ...
        df[['genre', 'loudness', 'energy', 'danceability']]
    ]
    ...
    plt.savefig('./ForHome5/DataScience/diagrams/roc_curve.png')
    plt.cla()

    img = openpyxl.drawing.image.Image(f'./ForHome5/DataScience/diagrams/roc_curve.png')
    img.anchor = 'A7'
    ws.add_image(img)
