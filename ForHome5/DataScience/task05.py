...
def base_model(y, ws):
    ...
    pred = np.bincount(y).argmax()

    y_pred = np.ones_like(y) * pred

    base_model_recall = recall_score(y, y_pred, average='macro')
    base_model_precision = precision_score(y, y_pred, average='macro')
...
def train_and_test_model(X, y, ws, name):
    ...
    cv = GridSearchCV(pipeline, param_grid=param_grid,cv=kf,return_train_score=True, scoring='f1_macro')
...
def main():
    ...
    y, uniques = pd.factorize(df['genre'])
    ...
    X_options = [
        ...
        df[['popularity', 'acousticness', 'danceability', 'energy', 'instrumentalness', 'speechiness']]
    ]
