...
def train_and_test_model(X, y, ws):
    ...
    if X_test.shape[1] == 1:
        ...
        plt.savefig(f'./diagrams/regression_diagram_{X_test.columns[0]}.png')
        plt.cla()

        img = openpyxl.drawing.image.Image(f'diagrams/regression_diagram_{X_test.columns[0]}.png')
        img.anchor = 'E5'
        ws.add_image(img)
...
def main():
    ...
    X,y = df[['tv', 'radio', 'social_media']], df['sales']
    train_and_test_model(X, y, wb['ModelReport'])
